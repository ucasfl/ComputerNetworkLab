...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	/*printf("in tcp_sock_lookup_established\n");*/

	int key = tcp_hash_function(saddr, daddr, sport, dport);
	struct tcp_sock *sk_pos, *sk_q;
	list_for_each_entry_safe(sk_pos, sk_q, &tcp_established_sock_table[key], hash_list){
		if (saddr == sk_pos->local.ip && daddr == sk_pos->peer.ip \
					&& sport == sk_pos->local.port && dport == sk_pos->peer.port){
			/*printf("leave tcp_sock_lookup_established\n");*/
			return sk_pos;
		}
	}
	/*printf("tcp_sock_lookup_established, not found\n");*/
	return NULL;
}
...
{
	/*fprintf(stdout, "TODO: implement this function please.\n");*/
	/*printf("in tcp_sock_lookup_listen\n");*/
	...
	/*printf("tcp_sock_lookup_listen, not found\n");*/
	return NULL;
}
...
{
	/*fprintf(stdout, "TODO: implement this function please.\n");*/
	/*printf("in tcp_sock_listen\n");*/
	tsk->backlog = backlog;
	tcp_set_state(tsk, TCP_LISTEN);
	tcp_hash(tsk);
	/*printf("leave tcp_sock_listen\n");*/
	return 0;
}
