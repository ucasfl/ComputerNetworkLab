		...
		struct stp_config *config)
{
	...
	{
		//update port config, modify designed to undesigned
		...
		// update switch 
		// find root port
		/*bool is_root_stp_before = stp_is_root_switch(stp);*/
		...
		for (int i = 0; i != stp->nports; ++i){
			if (not_de_port[i] && !strcmp(stp_port_state(&stp->ports[i]), "ALTERNATE")){
		...
		for (int i = 1; i != stp->nports; ++i){
			...
			if (not_de_port[i]&&!strcmp(stp_port_state(&stp->ports[i]), "ALTERNATE")&&
						(PRIOR(stp->ports[i].designated_root, root_port->designated_root,
						       stp->ports[i].designated_cost, root_port->designated_cost,
						       stp->ports[i].designated_switch, root_port->designated_switch,
						       stp->ports[i].designated_port, root_port->designated_port)))
			  root_port = &stp->ports[i];
		}
		//judge if designed port
		/*if (){*/
		/*}*/
		if (root_port){
			/*printf ("ROOT PORT : %d\n", root_port->port_id);*/
		...
		//update all designed port of the switch
		for ( int i = 0; i != stp->nports; ++i ){
			if (stp_port_is_designated(&stp->ports[i])){
		...
		/*bool is_root_stp_now = stp_is_root_switch(stp);*/
		if (!stp_is_root_switch(stp)){
			// root -> non root
			// stop timer
			stp_stop_timer(&stp->hello_timer);
		}
		//  send updated config from every designated port
		/*for (int i = 0; i != stp->nports; ++i){*/
		/*    if (stp_port_is_designated(&stp->ports[i])){*/
